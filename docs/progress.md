...
## 현재 상태 — M2C-02b 서버 연결의 로컬 검토본
기준 원격 커밋은 `e760831e1c0923be9d7b54124c6ff55399f76d05`다. 이전 영속 기록 보관본 49개 파일의 해시와 추가 원격 기준 소스 6개의 Git blob을 대조했다. 서버 설정·공유 상태·Workspace 선택으로 연결 코드를 작성했다. **코드 업로드 차단을 재시도·우회하지 않았고 이번 Git 변경도 문서·검증 JSON만이다.**
...
| M2C-02b-3 | 운영 상태 조회·검토 후 복구 도구 | 미구현 | 상태 요약은 복구 명령을 대체하지 않음 |
...
다음 관문은 허용된 코드 반영 경계와 전체 SDK/서버 통합 검증이다. 운영 상태 조회/검토 후 복구, 서로 다른 ID의 교차 프로세스 파일 잠금, 일반 쓰기 버전 계약, 다중 파일 트랜잭션, E0-D·M3는 남아 있다. 영속 모드도 ID/기준 해시 없는 작업까지 보호하지 않는다. 부모 relay 등록 순서, 전원/커널 장애, 자동 롤백/프로세스 부활을 검증했다고 주장하지 않는다.
